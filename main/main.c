...
#define EV1527_SYNC_LOW_US  (EV1527_T_US * 28) // Sync low duration (long pause) ~9450us //temporaray to test
...
static uint32_t decode_ev1527_signal(rmt_symbol_word_t *symbols, size_t num_symbols)
{
    ...
    uint8_t bit_count = 0;
    int expected_high_min = EV1527_LONG_PULSE_US-EV1527_T_TOLERANCE_US;
    int expected_high_max = EV1527_LONG_PULSE_US+EV1527_T_TOLERANCE_US;
    int expected_low_min = EV1527_SHORT_PULSE_US-EV1527_T_TOLERANCE_US;
    int expected_low_max = EV1527_SHORT_PULSE_US+EV1527_T_TOLERANCE_US;
    ...
    while ((symbol_index < num_symbols) && (bit_count < 28)) { // EV1527 is 28 bits total //TODO this passes ev code by 4 bits,make it go until hit the long 
    //pulse and also check 24 and 28 bits respectively according to dataSheet and implement these codes separately  
        ...
        ESP_LOGW(TAG, "shold nevere come here!!!!!!!!!!!!!!!!!!!!");
